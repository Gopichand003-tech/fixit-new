...
const ServiceCategories = () => {
  ...
  return (
    <section className="w-full bg-gradient-to-b from-gray-50 to-gray-100 py-12 sm:py-16 overflow-x-hidden">
...
<div className="text-center mb-16">
  ...
  >
    Discover trusted professionals across multiple categories near you.
        ...
        {/* Category Filter */}
        <div className="flex gap-3 sm:gap-4 mb-10 overflow-x-auto sm:overflow-visible justify-start sm:justify-center scrollbar-hide">
          {categories.map((cat) => (
            <button
              key={cat}
              onClick={() => setSelectedCategory(cat)}
              className={`whitespace-nowrap px-5 py-2.5 rounded-full text-sm sm:text-base font-semibold transition-all duration-300
                ${
                  selectedCategory === cat
                    ? "bg-gradient-to-r from-blue-500 to-blue-500 text-white shadow-md scale-105"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4 sm:gap-6">
          {filteredServices.map((service) => (
            <div
              ...
              className={`group cursor-pointer rounded-2xl p-5 sm:p-6 text-center
                bg-white/90 backdrop-blur-md border border-gray-200
                transition-all duration-300 ease-out
                hover:-translate-y-2 hover:shadow-xl active:scale-95
                ${selectedService === service.name ? "ring-2 ring-blue-500 shadow-lg" : ""}
              `}
            >
              <div
        ...
        <div className="mt-10 flex justify-center">
          <button
            onClick={() => navigate(-1)}
            className="px-6 py-3 text-sm sm:text-base bg-gradient-to-r from-blue-500 to-blue-500
              text-white rounded-full shadow-lg hover:scale-105 active:scale-95 transition-all duration-300"
          >
            ← Back
          </button>
        </div>
